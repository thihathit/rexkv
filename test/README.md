...
**SFE bundling tests, not functional tests.** They verify the `rexkv` `.node` addon embeds
...
| `bun-sfe-build_and_run/` | Current variant: `bun build --compile` embeds `native/rexkv.node` in Bun's VFS, then runs it. |
